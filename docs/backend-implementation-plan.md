...
- 削除系操作（プロジェクト削除・カテゴリ削除・メンバー削除・招待取消）は **Owner/Adminのみ**許可
- **タスク削除のみ例外**で、担当者本人 または Owner/Admin に許可（タスクは軽量運用前提のため、
  自分がアサインされたタスクは自分で片付けられるようにする）
- メンバーのロール変更（Owner/Admin/Member切替）も Owner/Adminのみ許可
- 追加ガードとして、**自分自身をメンバーから削除する操作**と、**組織にOwnerが1人もいなくなる
  操作（最後のOwnerの削除・降格）**をAPI側でブロックする（§5引き継ぎメモで論点として指摘されて
  いた箇所への回答）
- メンバーの**招待**（`POST /api/members`、招待自体の作成）は今回のロール制限のスコープに含めない
  （ユーザーへの確認質問が「削除・招待取消」であり「招待」自体は含めていなかったため。全ロールが
  引き続き招待できる。将来的に「Memberが誰でもOwnerとして人を招待できてしまう」ことが問題になる
  場合は別途スコープを切って検討する）
...
| ファイル                        | 内容                                                                                                                                                                                                              |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lib/auth/permissions.ts`        | ロール判定の共通ロジック。`MANAGE_ROLES`（Owner/Admin）、`canManageOrg(role)`、`canDeleteTask(canManage, currentUserId, assigneeId)`、`isOnlyOwner(members, userId)`。API Route Handler・UIの両方から参照し、判定ロジックを二重実装しない |
| `__tests__/permissions.test.ts`  | 上記4関数のロールごとの網羅的ユニットテスト                                                                                                                                                                       |

**変更したファイル（API側）**:

- `lib/api/auth.ts`: `requireOrgId()` が `userId`・`role`（Clerkの`orgRole`から`toRole`で変換）も返すように拡張。新たに `requireOrgRole(allowedRoles)` を追加（許可ロール以外は403）
- `lib/api/respond.ts`: `forbiddenResponse(message)` を追加（403の `{error}` レスポンス）
- `lib/clerk/org-members.ts`: `isSoleOwner(orgId, userId)` を追加（`listActiveMembers` を再利用し `isOnlyOwner` で判定。既存メンバー一覧取得ロジックを再利用し二重実装しない）
- `db/repositories/tasks.ts`: `getTaskById(orgId, id)` を追加（タスク削除時に担当者を確認するため、削除前に対象タスクを取得する必要が生じたため）
- `app/api/categories/[id]/route.ts`・`app/api/projects/[id]/route.ts`: `DELETE` を `requireOrgId()` → `requireOrgRole(MANAGE_ROLES)` に変更
- `app/api/tasks/[id]/route.ts`: `DELETE` は `requireOrgId()` のまま（担当者本人にも許可するため、ロール一律の`requireOrgRole`は使えない）。`getTaskById` で対象タスクを取得し、`canDeleteTask(canManageOrg(ctx.role), ctx.userId, task.assigneeId)` が `false` なら403を返す実装にした
- `app/api/members/[id]/route.ts`: `PATCH`（ロール変更）・`DELETE`（メンバー削除）を `requireOrgRole(MANAGE_ROLES)` に変更。`DELETE` はさらに「`id === ctx.userId`（自分自身）」「`isSoleOwner(ctx.orgId, id)`（唯一のOwner）」を弾く。`PATCH` は変更後の `role !== "owner"` かつ `isSoleOwner` なら弾く（`role === "owner"` への変更、つまり実質no-opは唯一のOwnerでもブロックしない）
- `app/api/members/invitations/[id]/route.ts`: `DELETE`（招待取消）を `requireOrgRole(MANAGE_ROLES)` に変更

**変更したファイル（UI側）**:

- `components/workspace/Workspace.tsx`: `useOrganization()`/`useUser()`（`@clerk/nextjs`）でログイン中ユーザーの `orgRole`・`userId` を取得し、`canManage = canManageOrg(toRole(membership?.role))` を計算。`ProjectListPane` に `canDeleteProject`、`GlobalHeader`（→`SettingsDialogContent`）に `canDeleteCategory`、`ProjectDetailPane` に `canManageOrg`・`currentUserId` として渡す（既存の `canAddProject` パターンを踏襲し、判定はpropsで受け渡すのみでWorkspace.tsxに一元化）
- `components/workspace/ProjectListPane.tsx`: `canDeleteProject` propを追加。プロジェクト削除の `DropdownMenuItem` に `disabled={!canDeleteProject}` を付与し、`Tooltip`（既存の`canAddProject`と同じ構成）で「この操作はOwner/Adminのみ実行できます」を案内
- `components/workspace/GlobalHeader.tsx`: `canDeleteCategory` propを追加し `SettingsDialogContent` へ中継
- `components/workspace/SettingsDialog.tsx`: `canDeleteCategory` propを追加。カテゴリ削除ボタンに `disabled`+`Tooltip` を追加
- `components/workspace/ProjectDetailPane.tsx`: `canManageOrg`・`currentUserId` propsを追加。「詳細」タブの `TaskDetailContent` に `canDelete`（`canDeleteTask(canManageOrg, currentUserId, task.assigneeId)`）を渡し、タスク削除ボタンに `disabled`+`Tooltip`（「このタスクの担当者、またはOwner/Adminのみ削除できます」）を追加
- `components/workspace/MemberManagementSection.tsx`: 独自に `useOrganization()` を呼び `canManage` を計算（`Workspace.tsx`からのprops経由にしなかった理由: このセクションは`SettingsDialogContent`のさらに下にあり、propsのバケツリレーを1段追加するより、既にロール表示で同じフックを使っている`OrgSwitcher.tsx`と同様に自己完結させる方がシンプルと判断）。メンバー削除ボタン・ロール変更`Select`・招待取消ボタンに `disabled={!canManage}`+`Tooltip`を追加。招待フォーム（メール入力・招待ボタン）は対象外（前述の通りスコープ外）
- `lib/labels.ts`: `MANAGE_ROLE_TOOLTIP`・`TASK_DELETE_ROLE_TOOLTIP`・`SELF_REMOVE_TOOLTIP`・`LAST_OWNER_TOOLTIP` を追加（後2つは現時点でUI文言としては未使用。自分自身の削除・最後のOwnerガードはAPI側の403のみで表現し、UI側は他の操作失敗時と同じ「楽観的更新→黙ってロールバック」方針に委ねたため。定数として用意だけしておき、将来トースト等のUI通知を追加する際に流用できるようにした）

**設計判断メモ**:

- **「自分自身の削除」「最後のOwnerの削除・降格」の2ガードはAPI側のみで実装し、UI側での事前disabled化はしなかった**。理由: 対象がメンバー一覧の特定の1行（＝ログイン中ユーザー自身の行、または唯一のOwnerの行）に限られる細かい条件分岐であり、`MemberManagementSection.tsx`は既に`runOptimistic`（失敗時は黙ってロールバック、`console.error`記録のみ）という既存の統一されたエラー方針に従っている。ここに新しいUI通知パターン（特定行だけ理由の異なるdisabled状態）を足すとUIの複雑度が増す一方、実際の被害（誤操作の可能性）はAPI側のガードで防げているため、既存の「失敗は黙ってロールバック」方針を踏襲する形にした
- **タスク削除の権限判定はRoleではなくブール値（`canManage`）を受け取る関数にした**（`canDeleteTask(canManage: boolean, ...)`）。API側は`ctx.role`から`canManageOrg(ctx.role)`を計算して渡し、UI側（`Workspace.tsx`）は元々`canManageOrg(toRole(membership?.role))`というブール値しか保持していない設計にしたため、Role型を経由させるより自然だった
- **招待（`POST /api/members`）は制限対象に含めなかった**（前述の事前確認事項参照）。実装上は`requireOrgId()`のまま変更していない

**確認・検証状況**:

- `npm run test`（164件）・`npm run lint`・`npm run build` はいずれもグリーン（ローカルのNodeがv20.10.0だと`vitest.config.ts`の読み込みに失敗するため、Node 23.7.0に切り替えて実行した。§1のメモの通り既知の問題）
- ブラウザでの実動作確認は、§5と同様の理由でサンドボックス化されたプレビューブラウザからは実施できなかった（Clerkのホスト型サインインページへの外部ドメイン遷移が`net::ERR_ABORTED`で失敗するため）。ロールに基づく表示切り替え（Owner/Admin/Memberでの削除ボタン・ロール変更セレクトのdisabled状態）は、`__tests__/member-management.test.tsx`に追加したテスト（Memberロール時に削除ボタン・ロールセレクト・招待取消ボタンが`disabled`になることを検証）でカバーしている。ユーザー自身の手元環境で実際に複数ロールのユーザーを用意し、`npm run dev`から動作確認することを推奨する
- README.md / CLAUDE.md をバックエンド実装フェーズ完了後の状態に更新した（環境構築手順・環境変数一覧・Vercelデプロイ手順を追記）。`docs/mock-implementation-plan.md` §8 も本セクションの完了を反映済み

**このドキュメントの状態**: セクション1〜6すべて完了。バックエンド実装フェーズはこれで一区切り。今後の追加作業（プロジェクト単位の権限、招待自体のロール制限見直し等）は、新たに本ファイルにセクションを追加して進めること。
