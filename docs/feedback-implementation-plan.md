...
- 2026-07-04: ステップ4-Aのヒアリングを完了し、9つの論点（タスク階層モデル / カテゴリの扱い / Pane1 / Pane2 / Pane3 / カレンダービュー配置 / 進捗%算出 / 完了タスクUI / 既存データ移行）の決定を `docs/pane-redesign-decisions.md` に記録した。
- 4-B以降は `docs/pane-redesign-decisions.md` を実装前提として読むこと。特に、カテゴリはDB/UIともに廃止、タスクは単一 `tasks` テーブルの `parentTaskId` + `level` による最大3階層、既存タスクは各プロジェクトのデフォルト大項目「未分類」配下の小項目へ移行する。
...
- `docs/pane-redesign-decisions.md` の決定に従い、DB上のカテゴリを廃止した。`db/schema.ts` から `categories` テーブルと `projects.categoryId` を削除し、`drizzle/0003_sharp_leader.sql` で `categories` テーブル削除・`projects.category_id` 削除を行う。
- タスク階層は単一 `tasks` テーブルに `parentTaskId` と `level`（`large` / `medium` / `small`）を持たせる形にした。`tasks.parent_task_id` と `tasks.level`、`task_level` enum、`tasks_parent_task_id_idx` を追加済み。
- `lib/schema.ts` に `taskLevelSchema` / `TaskLevel` と `Task.parentTaskId` / `Task.level` を追加した。4-CでUI/APIを切り替えるまでの移行期間だけ、旧UI互換として `Category` 型・`categoriesSchema`・`Project.categoryId` は deprecated コメント付きで残している。
- 既存データ移行スクリプト `scripts/migrate-task-hierarchy.ts`（`npm run migrate:task-hierarchy`）を追加した。各プロジェクトにデフォルト大項目「未分類」（ID: `default-large-${projectId}`）を作り、既存タスクはその配下の小項目へ移す。冪等実行可能。
- 実DBに `npm run db:migrate` を適用し、続けて `npm run migrate:task-hierarchy` を実行済み。実行結果は `projects=9 createdParents=9 updatedChildren=25`。
- `db/seed-data.ts` / `db/seed.ts` はカテゴリ投入をやめ、シード時も各プロジェクトに「未分類」大項目を作って旧JSONタスクを小項目化するよう更新した。
- 4-C前のビルド互換のため、`db/repositories/categories.ts` はDBアクセスしない deprecated shim として残した。Route Handler本体とPane1〜3 UIは本ステップのスコープ外のため変更していない。
- 追加・更新したテスト: `__tests__/db-schema.test.ts`、`__tests__/db-seed-data.test.ts`、`__tests__/db-integration.test.ts`、`__tests__/api-tasks.test.ts`。実DB統合テストでは `categories` が消え、`tasks.parent_task_id` / `tasks.level` が存在することを確認する。
- 検証: `npm run test`（207件、Neon統合テスト込み）/ `npm run lint` / `npm run build` はすべてグリーン。ローカルの `/usr/local/bin/node` は既知のESM問題があるため、検証は Codex 同梱 Node v24.14.0 で実行した。
