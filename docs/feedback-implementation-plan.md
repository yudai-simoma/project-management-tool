...
| 5   | 全体ダッシュボードの再設計（ヒアリングベース）      | 4-E推奨   | ✅ 完了  |
...
- 全体ダッシュボードの主目的は **リスク優先** にする。プロジェクトとタスクの両方を対象に、期限超過・期限間近を最初に判断できる画面にする。
- KPIは `期限超過プロジェクト` / `期限間近プロジェクト` / `期限超過タスク` / `期限間近タスク` / `平均進捗%` / `未完了タスク数` の6つにする。
- 色判定は、未完了かつ期限超過を赤、未完了かつ7日以内を黄、それ以外または完了済みを緑にする。実装では既存 semantic token の `destructive` / `chart-3` / `chart-2` を使い、生の色番号は使わない。
- プロジェクト一覧はリスク順で全件表示する。並び順は赤 → 黄 → 緑、同じリスク内では期限が近い順にする。
- 直近期限タスクは未完了のみ、期限順、上位10件を表示する。
- メンバー別ワークロードを表示し、未アサインも1行として出す。
- AIによる全体サマリーは今回のスコープ外とし、既存データからの派生計算だけで見えるダッシュボードにする。
- プロジェクト行クリックで該当プロジェクトへ移動し、タスク行クリックで該当プロジェクト・タスク詳細へ移動する。

**実装内容**:

- `lib/computed/projects.ts` に `getPortfolioDashboardSummary` と関連型を追加し、KPI、リスク順プロジェクト一覧、直近期限タスク、メンバー別ワークロード、ステータス内訳を既存の進捗%・期限リスク計算から派生するようにした。
- `PortfolioDashboardPane.tsx` を再設計し、上段KPI、リスク順プロジェクト全件、直近期限タスク10件、メンバー別ワークロード、補助のステータス内訳を表示する構成にした。
- 赤/黄/緑のリスク表示は `components/ui/badge.tsx` に `warning` / `success` variant を追加して扱い、呼び出し側で生の色クラスを直接使わないようにした。
- `Workspace.tsx` から `members` と遷移ハンドラを `PortfolioDashboardPane` に渡し、ダッシュボードのプロジェクト行・タスク行からワークスペース表示へ戻れるようにした。
- `__tests__/projects.test.ts` に集計ロジックの単体テストを追加し、`__tests__/portfolio-dashboard.test.tsx` に表示・クリック導線のコンポーネントテストを追加した。

**検証**:

- Codex同梱Nodeで `npm run test`（218件、Neon統合テストはネットワーク許可付き）/ `npm run lint` / `npm run build`（Google Fonts取得のためネットワーク許可付き）をグリーン確認済み。
